...
class TwitterSpider(object):
    ...
    def filter_keyword(self, _type, _dic, data=None):
        ...
        try:
            result = self.es.dsl_search(self.es_name, _type, mapping)
            if result.get("hits").get("hits"):
                if _type == "detail_type":
                    self.es.update(self.es_name, _type, result.get("hits").get("hits")[0].get("_id"), data)
    ...
    def save_one_data_to_es(self, data, dic):
        ...
        try:
            ...
            if self.filter_keyword(_type, dic, data):
                ...
                return
            self.es.insert(self.es_name, _type, data)
    ...
    def get_comment_data(self, data_min_position, twitter_id):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if flag_tag:
                    resp_dic.update(twitter_id=twitter_id)
                    return resp_dic
        except Exception as e:
            self.requester.use_proxy()
            raise HttpInternalServerError
...
if __name__ == '__main__':
    ...
    for pro_dic in profile_tweet_data_list:
        ...
        comment_list.append(dict(twitter_id=twitter_id, items_html=pro_dic.get("resp")))
        if pro_position:
            for i in range(1, 20):  # 一会换
