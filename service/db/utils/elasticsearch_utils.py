...
class ElasticsearchClient():
    ...
    def __init__(self):
        ...
        es_host = "{}:{}/".format(s_conf["url"], s_conf["port"])
        h_es_host = 'http://{}:{}@{}:{}'.format(h_conf["user"], h_conf["password"],  h_conf["host"], h_conf["port"])
        self.es = Elasticsearch(hosts=es_host)
        self.h_es = Elasticsearch(hosts=h_es_host)
    ...
    def create_index(self, index_name, index_mappings, ignore=400):
        ...
        try:
            ...
            if self.h_es.indices.exists(index=index_name) is not True:
                result = self.h_es.indices.create(index=index_name, body=_index_mappings, ignore=ignore)
...
h_es_client = ElasticsearchClient()
